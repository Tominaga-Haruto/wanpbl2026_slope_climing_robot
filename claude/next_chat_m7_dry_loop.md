...
- `claude/chats/YYYY-MM-DD_m7-onnx-golden-dry-run.md` を作り、README索引へ加える。
- `Connect2USB2CAN` のgit statusを確認し、対象スクリプトとテストだけを扱う。重み、ログ、仮想環境はコミットしない。
- 次段階へは進まず、action／目標角の最大誤差、周期ログ、残った統合条件を報告する。

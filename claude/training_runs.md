...
> run ごとに1行。**2026-09-16 より前の run（旧機体）は無効なので載せない。** 設計の根拠と結果の解釈は `crab_standstill_countermeasures.md`（§3 設計、§4 結果）。**用語（S1〜S9 など）の説明と経緯の読み物は `wrs_training_history.md`。**
> 判定は `tools\measure_crab.py`（平地・固定指令・64 env・2 s 捨てて 10 s）と、事前に決めた基準（同文書 §3）。**実験03 から小旋回（S7/S8）とトルク（S1 で RMS ≤ 定格、最大 ≤ ピーク、飽和率と computed torque 併記）を追加**（`next_chat_briefing.md` §1a の決定）。
...
## 実験01（2026-09-16 05:15〜、`wrs_experiment01_instruction.md`）
...
## 実験02（2026-09-16 午後〜、`wrs_experiment02_instruction.md` の P3 を `wrs_experiment02_p3_instruction.md` で差し替え）
...
| ~~D_noStd / D_noBiped / B_combined_s2~~ | | 中止（`crab_standstill_countermeasures.md` §5-3） | | | |
...
## 実験03（2026-09-16 17:28〜21:12、`wrs_experiment03_instruction.md`、報告 `tools\logs\REPORT_exp03_final.md`）
...
## 実験04（2026-09-16 22:09〜、`wrs_experiment04_instruction.md` ＋ `wrs_experiment04_followup_instruction.md`、報告 `tools\logs\REPORT_exp04_stop2.md` / `REPORT_exp04_stop3.md`）

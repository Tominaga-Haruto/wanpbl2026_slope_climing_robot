...
**D7両CAN原点設定（2026-09-20）:** ユーザーが最終構成を完璧な基準姿勢で機械固定した状態で、ch=0の左脚5軸、ch=1の右脚5軸を同時に扱う専用コンソールから10軸すべてへモード5の原点設定を送信した。ログでは全軸の直後が`pos=+0.0deg`・`err=0`（30〜34°C）。**D7とD4の原点対応は完了**し、全軸の変換は`q_H=q_MIT`／`q_MIT_target=q_H_target`である。設定前角度・写真・支持寸法がログに無い点だけは記録上の注意として残す。電源断、姿勢・構成変更後は必ず同じ基準姿勢でD7を再実施する。正本は`deployment_roadmap.md`と`reports/2026-09-20_d4-m5-joint-map-and-origin-procedure.md`。

**D8 ver9統合・送信器（2026-09-21）:** CAD確定の`R_OFFSET=(+0.06345,+0.08900,+0.04275) m`を適用し、42要素観測→ONNX→H順10目標角→両CANの10軸mode 8 MIT送信まで実装した。AK10/AK80の実測c_p/c_dでKp/Kdを換算し、`--arm`なしでは送信せず、終了・例外時は全軸零MITフレームを3回送る。17テストと`--preview`は合格。実機は未起動。D9は `d9_吊り10軸MIT実験手順.md` に従うだけで、D9中の新規実装はしない。

...
**D5コントローラー乾式確認（2026-09-20）:** Jetson通常ターミナルで**直結時だけ**Switch 2 Proを50 Hzで読み、中立・前後左右・抜線・再接続をCAN/ONNX/T265/モーター未接続で確認した。左スティックは前=`+vx`、後=`-vx`、右=`-vy`、左=`+vy`、`wz=0`。実運用に必要な変換ケーブル経由ではPro Controller `057e:2009`がUSB設定`error -32`で再接続を繰り返し、`/dev/input`が作られない。**D5は実運用の延長経路で未完了。** 直結と同じ`045e:028e`で安定認識するUSBデータ対応の延長経路を確保してから再試験する。抜線ではプローブが`Errno 19`で終了するため、D8/D9の統合ループ側で入力途絶時のゼロ指令・安全停止継続を実装する。Jetson側Codexには`/dev/input`が公開されない。

...
- **D8送信器（2026-09-21）:** `C:\Users\harut\Connect2USB2CAN\ver9_d8_sender.py` を実装済み。T265 offset、両CAN ch=0/1、10軸H順、ONNX、実測c_p/c_d換算、50 Hz mode 8 MIT送信、終了時零MIT3回を含む。`--preview`・構文確認・17テストに合格。`--arm`なしでは送信しない。実機未起動。D9は `d9_吊り10軸MIT実験手順.md` の実行のみ。

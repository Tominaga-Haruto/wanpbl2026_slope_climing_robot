...
- `claude/PROJECT.md`: 基本情報、実行環境、文書構造を定義する。
...
既存の`README.md`、`next_chat_*`、`chats/`は削除・移動していない。旧`project_handbook.md`は`legacy/project_handbook_legacy.md`へ保全し、元パスには新構成への互換案内を置いた。新規セッションの開始点からは外し、新しい実測は`reports/`へ根拠を残し、優先作業が変われば`CONTEXT.md`と`handoffs/ACTIVE.md`を同じcommitで更新する。

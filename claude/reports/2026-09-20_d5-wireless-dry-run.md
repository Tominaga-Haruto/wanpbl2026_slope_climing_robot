...
**完了（2026-09-20、ユーザー確認）:** 本書の手順で登録削除からのペアリング、信頼、接続を繰り返し再現できた。D5 の実運用接続経路は Bluetooth として受け入れる。途中の直後切断はコントローラーの電池残量が要因の可能性があるが、切断原因の確定記録はない。D8 の開始前には、本書の「現在の入力イベントの解決」を必ず再実施する。
...
Bluetooth 接続、MACで照合した現在の入力イベント、上の4項目がすべて通れば、D5 は実運用経路で完了と記録する。2026-09-20 にユーザーが本手順での再現可能な接続を確認し、D5 の実運用接続経路を完了として受け入れた。D8 の入力境界へ接続する前にも、同じ Jetson・同じ無線経路で `eventN` を再確認する。

...
- USB2CAN認識後、`log on` の画面ログとCSVを取得した。ID18 / 19 / 27 / 34 / 42の5台が約50 Hzで定期フィードバック。ID34/18の `dual 0 0 2` は100周期完走し、`dual 0.2 0.2 2` で操作者が二機同時の実動を確認した。
- 電流・速度・移動量の各中止条件も実機で発動し、そのたび両IDへ零MIT指令が送られた。具体値は `motor_can_findings.md` の09-19節を正本とする。

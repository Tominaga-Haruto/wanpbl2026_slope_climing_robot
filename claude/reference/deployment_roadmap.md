...
| D8 | ver9統合ループ・D9用10軸送信器 | **MIT再開ブロック**。T265起動は修正済み。ユーザー確認で左右ID表が逆と判明し、共有対応表へ修正済み。T265接続時のUSB2CAN両ch初期化が未解決 | **D1, D2, D3, D4, D7** | `C:\Users\harut\Connect2USB2CAN\robot_joint_map.py`、`procedures/d9_1_usb_t265_dualcan_受信実験手順.md` |
...
| D10 | 床に下ろし、**固定**速度指令で平地の初回統合を確認する | 未実施 | **D9** | `procedures/d10_床上_平地デプロイ_実験手順.md` |
| D11 | Jetson GPU・無線プロコンによる平地デプロイを確認する | 実装・無通電単体試験済み、実機未実施 | **D10、Jetson無通電受入** | `procedures/d11_jetson_無線操作_平地デプロイ実験手順.md` |
...
- **D8送信器:** `ver9_d8_sender.py` は `--arm` がない限りCANを送信しない。T265の最初のposeを最大5秒待ち、10軸の新鮮なフィードバック、`err`、電流、速度を確認してから送信する。終了・例外時は零MITを3回送る。現在はMITを送らず、`procedures/d9_1_usb_t265_dualcan_受信実験手順.md`でT265接続時のUSB2CAN両ch初期化を確認する。

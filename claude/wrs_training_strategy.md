...
> **★この文書は方針のたたき台。** 実際の実験計画は、学習戦略のチャット（`next_chat_briefing.md`）でユーザーと決める。
...
- **base の +X（Isaac Lab の「前」）がロボットの横方向だった。** 08-21 版 URDF でも同じ（`robot_model_conventions.md`、手順書 B2）。
...
- 制御周期と実機 50 Hz の整合
- 関節名 ↔ モーターID ↔ 符号の対応表（`robot_model_conventions.md` を基準に）

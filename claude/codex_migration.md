...
> 関連: `AGENTS.md`（リポジトリ直下に置く入口）、`next_chat_briefing.md`、`next_chat_briefing_motor.md`
...
`README.md`, `project_handbook.md`, `handover.md`,
`next_chat_briefing.md`, `next_chat_briefing_motor.md`, `controller_prep_briefing.md`,
`robot_model_conventions.md`, `motor_can_findings.md`, `realsense_t265.md`, `actuator_params.md`,
`wrs_pc_environment.md`, `motor_bench_checklist.md`, `training_runs.md`
（＋この `codex_migration.md`）
 
**Tier 2 ── 判断の根拠。余裕があれば（8 本）**
 
`crab_standstill_countermeasures.md`, `wrs_training_strategy.md`, `wrs_training_history.md`,
`motor_mit_official_notes.md`, `project_structure_map.md`, `rough_env_cfg_walkthrough.md`,
`urdf_asymmetry_finding.md`, `template_code_reference.md`
 
**Tier 3 ── 経緯の保存。1週間の作業には要らない**
 
`chats/*`（24 本）、`wrs_experiment01〜09_instruction.md`、旧版（`urdf_reexport_instruction.md`, `wrs_pc_operator_instruction.md`, `mit_implementation_briefing.md`, `alienware_repair_instruction.md`, `wrs_pc_host_instruction.md`, `wrs_new_chat_start.md`, `wrs_overnight_20260918_instruction.md`, `isaaclab_edit_guide.md`, `onshape_cad_fix_instruction.md`）
...
- 読む順番（`claude/README.md` → `project_handbook.md` → 担当の引き継ぎ書 → 正本の md）
...
ノートPC 側にもリポジトリを置けば、同じ `AGENTS.md` がそのまま効く。
...
- PAT はチャットに貼らない。push 後にハードリンク5組の `Get-FileHash` を確認（`wrs_pc_environment.md`）。
...
- 実測ログ（`session_214916.txt` など）は、必要な数行だけ `motor_can_findings.md` に転記する。生ログは git に入れない。
...
- **`wrs_new_chat_start.md` を貼る儀式は不要になる**（あれは Claude Code 向け）。
...
- **残りの時間は実機に寄せる。** 制御ループ ver9 の乾式テスト → 吊った状態で1モーター → 脚1本 → 全身（`next_chat_briefing_motor.md` の M1〜M9）。
---

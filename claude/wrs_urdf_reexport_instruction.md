...
> 作成 2026-09-15。旧版 `urdf_reexport_instruction.md`（Alienware / Ubuntu 前提）を **WRS機（Windows 11）用に作り直したもの。**
...
### 0-3. 構築済みの環境（`wrs_pc_environment.md` より）
...
3. 自作5ファイルのハードリンクが生きているか（`my_robot_code\\` と参考実装側の `Get-FileHash` 比較。対応表は `wrs_pc_environment.md` §1）

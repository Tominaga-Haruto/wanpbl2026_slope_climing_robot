...
**2026-09-22 の更新:** H `model_19998.pt`から追加10000 iterのA/B@29997をflat・64 env・seed1234で評価。A自動headingは不要yawが直進・静止・後退にも混入して不採用。B直接`(vx,vy,wz)`はS6=+0.445/S9=-0.310 rad/s、転倒0で有望だが、256 env×2 seed・連続300 iter・H同等頑健性という採用規則が未達である。第一回実機デプロイは検証済みH_eff13p5@2999のまま維持し、次はB既存保存点を資格評価する。詳細は`reports/2026-09-22_exp10-autoturn-directturn-assessment.md`。

**2026-09-22 夜間GPU枠:** B@29997から2本を追加9000 iter並走する予定。`B_continue_H30000`は変更なしの延長対照、`B_lateral10_H30000`は純旋回38%を維持し横移動指令10%を追加する実験である。本番はユーザーが前景PowerShellから開始する。実機候補のH@2999はこの計画では置換しない。
...
**Hデプロイ成果物（2026-09-19）:** H_eff13p5@2999 の既存ONNXから、ONNX・観測契約・golden dataset・アクチュエータ表をWRS側で抽出し、ノートPCへ受領した。SHA256は6必須ファイルすべてWRSのmanifestと一致、ONNXは42入力・10出力、Torchとの最大絶対誤差はrandom `9.537e-06`／実観測 `7.153e-07`でPASS。ノートPCの未追跡 `H_eff13p5_2999\\H_eff13p5_2999\\` は重みを含むためGitに追加しない。M7のCANなし乾式実装は完了（action最大 `1.1920929e-06`、目標角最大 `5.96046448e-07`、各≤`1e-4`、50 Hz CSV 500行）。M4は独立工程にせず、実機CANへの接続はまだしない。

**D5コントローラー乾式確認（2026-09-20、完了）:** Jetson通常ターミナルで有線直結のSwitch 2 Proを50 Hzで読み、中立・前後左右・抜線・再接続をCAN/ONNX/T265/モーター未接続で確認した。左スティックは前=`+vx`、後=`-vx`、右=`-vy`、左=`+vy`、`wz=0`。有線延長経路はUSB設定`error -32`で未解決だが、ユーザー判断でBluetooth無線を採用した。Bluetooth HID `98:B6:E9:4A:87:92`（`Pro Controller`、`057e:2009`）は登録削除からペアリング・信頼・接続を繰り返し再現できる。Bluetoothは`/dev/input/by-id`を作らないため、D8の前には`Uniq=98:b6:e9:4a:87:92`で現在の`eventN`を再検出する。抜線時のプローブは終了するため、D8/D9の統合ループ側で入力途絶時のゼロ指令・安全停止継続を実装する。無線接続の正本は`reports/2026-09-20_d5-wireless-dry-run.md`。

**D7両CAN原点設定（2026-09-20/21）:** 基準姿勢で全10軸へモード5原点を送り、直後は`pos=+0.0deg`・`err=0`だった。しかし電源断・姿勢復帰後にAK80-9で最大168.2°のずれを確認した。公式現行manualは`kind=1`恒久ゼロをデュアルエンコーダ機種限定とし、AK80-9 V3.0はSingleであるため、AK80を含む全身原点は電源断後に再利用できない。D9/D10の直前に、機械ゼロ姿勢で全10軸へ`kind=0`を設定し、主電源を切らず姿勢・配線を変えない同一通電中だけ有効とする。後の連続受信で、`gs_usb` reset後のPython ch=0/1列挙は左右で入れ替わりうるとも判明した。ID→chは各プロセスの受信で確定し、固定値にしない。正本は`motor_can_findings.md`、`d7_棒支持_全関節原点設定_実験手順.md`、`deployment_roadmap.md`。

**D8/D9 ver9統合・送信器（2026-09-21）:** CAD確定の`R_OFFSET=(+0.06345,+0.08900,+0.04275) m`を適用し、42要素観測→ONNX→H順10目標角→**USB2CAN V3.3 1台が公開するch=0/ch=1**の10軸mode 8 MIT送信まで実装した。AK10/AK80の実測c_p/c_dでKp/Kdを換算し、`--arm`なしでは送信せず、終了・例外時は全軸零MITフレームを3回送る。T265起動の`No device connected`は修正済み。D9-0で全IDの左右対応が従来表と完全に逆と判明したため、`robot_joint_map.py`を唯一の正本としてD7・ver9・現在の手順書を修正した。現在のブロックは、T265を接続した状態のUSB2CANが初回起動で片chだけになる／resetに失敗する問題である。D7はT265を読取していないため、T265データ量ではなくUSB初期化経路を検証する。全身MITを再実行せず、次は送信ゼロの`d9_1_usb_t265_dualcan_受信実験手順.md`を実施する。正本は同手順と`chats/2026-09-21_d9-usb-dualcan-and-id-map-handoff.md`。

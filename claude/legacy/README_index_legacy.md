...
WRS 側の CLI は、長い自走・定期監視・自動評価・大量のログ出力を依頼しない。必要なときは、実在する run・params・起動方法を**読むだけで確認**させ、返答は原則として「フォアグラウンド train の完成1行」と「固定 checkpoint の play の完成1行」だけにする。コード変更、テスト起動、学習開始、監視はユーザーがその都度明示した場合だけ依頼する。これは CLI のトークン消費を抑え、長時間タスクで応答が失われるのを避けるための運用規約である。

---

## チャット記録を作るタイミング

> `chats/` の記録は、**ユーザーから「引き継ぐ」と言われたときだけ**作成する。通常の会話・作業の終わりには新規作成しない。

- **タイミング:** ユーザーが「引き継ぐ」と明示したとき、返事を返す前に作る。
- **書き方:** 下の「`chats/` ── 個々のチャットの記録」節のテンプレートに従う。
- **セットでやること:** この README 末尾の**索引に1行追加**する。
- **通常時:** 作業内容が次の担当に渡る段階になるまで、記録ファイルと索引は更新しない。
...
5. ユーザーから「引き継ぐ」と言われた場合だけ、冒頭のルールどおり `chats/` に記録を追加する。
...
- **旋回実験10（2026-09-22、最終評価）:** H@19998から+10000 iterのA/B@29997を評価。A自動headingは不要yawが直進・静止にも混入し不採用。B直接`(vx,vy,wz)`は64 env/seed 1234で純旋回基準を満たしたが、256 env×2 seed・連続300 iter・頑健性の採用規則は未達。第一回実機は検証済みH@2999を維持し、次はBの既存保存点を資格評価する。→ `reports/2026-09-22_exp10-autoturn-directturn-assessment.md`

- **夜間GPU学習（予定）:** B@29997から+9000 iterを2本並走する。変更なし延長の`B_continue_H30000`と、純旋回38%を維持して横移動10%を追加する`B_lateral10_H30000`。64 env smoke test後、ユーザーが前景PowerShellで起動する。H@2999の実機候補は置換しない。

- **D8/D9送信器（2026-09-21、MIT再開ブロック）:** T265の`No device connected`は修正済み。T265接続時の`gs_usb` resetはランダムに失敗し、成功時にもPythonのch=0/1列挙が入れ替わる。関節ID（`0x2A=LR_HFE`を含む）の左右対応はユーザー確認どおり固定だが、USB chは固定値として使わない。`robot_joint_map.py`はIDだけの正本にし、D7・ver9・D10/D11送信器は各プロセスで10軸の新鮮な受信からIDごとの送信経路を確定する。経路確定前はMITを送らず、reset失敗は送信なしで1回だけ再試行する。D10／D11／ver9のMIT送信は、T265接続のままD7を新規プロセスで2回連続、10軸・err=0・age≤0.3 sで受信合格するまで実行しない。→ `d9_1_usb_t265_dualcan_受信実験手順.md`

- **原点は別の必須ゲート（2026-09-21）:** D7の`scan`成功は通信・ID経路だけの合格で、原点の合格ではない。9/20の`o 0`（モード5・起動中の基準原点）は電源をまたいで失われ、今の`±130〜170°`はID/ch修正前から出ていた未原点化座標である。公式ドライバマニュアルは`kind=1`恒久原点を**デュアルエンコーダ機専用**と明記し、AK80-9 V3.0はシングルエンコーダであるため、全10軸の電源断後恒久原点は作れない。D9送信器は絶対値45°超の関節を検出すると、MITを1フレームも送らず中止する。D9は既決の機械ゼロ姿勢でD7の`o 0x..`を全10軸へ行った**同一通電中**だけ実施する。
...
> **ユーザーから「引き継ぐ」と言われたときだけ**1ファイル追加する。通常の作業中・作業終了時には追加しない。
...
- **引き継ぎ1回＝1ファイル。** 1回で複数トピックを扱ったら分けてよい。長く続く作業は日付を分けてよい。
...
- **引き継ぎを求められた場合:** 体系的記録の更新とは別に、`chats/` への記録も作る。

### 索引（新しい順）
- chats/2026-09-22_d9-current-vs-error-diagnosis.md ── **D9 0x1C の停止は、MIT経路の異常ではなく試験の設計だった。** 保存済みCSVの回帰で `電流 = 8.2 A/rad × 位置誤差`（指令Kp 7.937、比1.03）＝MITトルク経路は正常と確定。MIT の Kp 欄は「1 rad あたりの電流[A]」として効く。不感帯 = 動き出し電流 ÷ Kp_cmd で 0x1C は 4.8°以上あり、要求角 4.52° / 2.5° は裸の AK80-9（動き出し 0.63 A）でも満たせない条件だった。送信前に `repeat-probe abort` する門と `--analyze` を実装。次は無通電の機構確認 → D9-3（要求角 6.5°、要承認）。学習側は friction 3〜5倍のスイープ。
- chats/2026-09-22_d9-one-axis-ramp-handoff.md ── **D9は1軸0x1Cだけを再試験中。** 初回ランプは401指令・8.016秒で実行済みだが、小角度のため視認だけでは動作を判定できない。初回方策出力の符号反転は送信目標をslew制限し、次版のCSVで実測位置・速度・電流を判定する。片脚・全身には進まない。
- chats/2026-09-21_d7-rework-and-d9-debug-handoff.md ── **D7は通電ごとに再実施。** 公式現行manualは`kind=1`恒久ゼロをデュアルエンコーダ機種限定とし、AK80-9 V3.0はSingle。電源断後にAK80の大角度ずれを実測したため、D9は機械ゼロ姿勢で全10軸`kind=0`を設定した同一通電中だけ進める。USB ch=0/1は固定せず、D9の次は送信ゼロの受信試験。
- chats/2026-09-21_d9-usb-dualcan-and-id-map-handoff.md ── **D9は未合格。** T265接続時にUSB2CANの初回起動が片chだけになる／resetに失敗する。T265データ量ではなく`gs_usb`のUSB初期化・reset経路を検証する。全IDの左右対応は旧表と完全に逆と確定し、共有の`robot_joint_map.py`と現行手順書を修正済み。次は送信ゼロの`d9_1_usb_t265_dualcan_受信実験手順.md`で新規プロセス2回の両ch受信を確認する。
- chats/2026-09-21_d9-t265-startup-blocker-handoff.md ── **D9はMIT送信前のT265起動でブロック。** 単体`t265_check.py`は3回連続成功したが、D9と送信なし統合の両方で`RealT265.start()`が`No device connected`。成功版がcontext/device参照を解放してからpipelineを開始する差が最有力。USB2CANは1台がch=0/ch=1を公開する構成で、D7の全10軸受信済み。
- chats/2026-09-21_autoturn-directturn-evaluation-handoff.md ── 実験10の実装・実行引継ぎ。
- chats/2026-09-22_exp10-final-evaluation.md ── **実験10の最終評価。** Aは不採用、Bは資格評価待ち。第一回実機はH@2999を維持。
- chats/2026-09-20_d7-dual-can-origin-handoff.md ── **D7実行待ち。** 10軸はch=0（左脚5軸）/ch=1（右脚5軸）へ分かれていることを受信で確認し、両チャネルを同時に扱うD7専用コンソールと16進数IDの手順書を追加した。次は棒支持・D6開始判定後に`scan 3`→10軸の`o`原点設定を行い、ログ・設定前後角度・写真・寸法を次チャットへ渡す。
- reports/2026-09-20_d5-wireless-dry-run.md ── **D5完了。** 有線延長のUSB設定失敗を回避し、Bluetooth HID `98:B6:E9:4A:87:92`（Pro Controller）の登録削除→ペアリング→信頼→接続を繰り返し再現できることを確認した。接続ごとに`Uniq`から現在の`eventN`を再検出する。CAN・ONNX・T265・モーターは未接続。
- chats/2026-09-20_d5-extension-path-handoff.md ── 有線延長経路のUSB設定失敗の経緯。Bluetooth実運用経路の採用により、D5のブロックではない。
- chats/2026-09-20_d5-jetson-dry-run-handoff.md ── D5の直結時の50 Hz乾式入力（中立・4方向・抜線検出・再接続後のゼロ復帰）は確認済み。CAN等には未接続。実運用の延長経路の問題は次の引き継ぎ書へ分離。
...
- chats/2026-09-19_d5-controller-mapping-handoff.md ── D5完了。固定／controllerの共通速度コマンド境界をCAN等なしで追加し、固定CSV・未確定mappingのゼロ出力・範囲外拒否・5件のテストを確認。次はM7の固定コマンド接続、またはJetsonでのpad_probe乾式確認と座標符号のユーザー確認。

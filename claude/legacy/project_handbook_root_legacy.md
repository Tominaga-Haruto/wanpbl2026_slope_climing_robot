...
最終更新: 2026-09-18未明（実験08 track_ang_vel_z_exp調整で初めて合格チェックポイントが出た直後）
...
- **実験07で追加診断（2026-09-17夜、`docs/experiments/exp07_hang_and_turn_diag.md`）**:
  ...
  → **実験08でtrack_ang_vel_z_expのstd/weightのみ変更して追試（2026-09-17夜〜18未明）**:
  std=0.35+weight=1.0（L_angstd_w1）の+1000iter地点（`model_4000.pt`）で**実験06の判定基準を
  プロジェクト初めて満たした**（S6 yaw=+0.268、S9 yaw=-0.353、比率0.76、転倒率0、両足着地、
  既存基準への退行なし）。ただし同じrunの+1499（最終）では再び基準を外れ、一時的な山だった
  可能性がある。std=0.35のみ（weight据え置き、L_angstd）は3チェックポイントとも不合格。
  ONNX書き出し・verify_onnx.py検証はPASS済み（`tools/logs/REPORT_exp08_stop8b.md`）。
  **次: 再現性確認（別seedでの再学習）と実機投入前のP1頑健性スイープが必要。**
...
| その場旋回込みの候補（未検証・再現性未確認） | `2026-09-17_21-18-22_L_angstd_w1` / `model_4000.pt`（実験08、ONNX書き出し済み。同runの+1499では基準を外れるため要注意） |

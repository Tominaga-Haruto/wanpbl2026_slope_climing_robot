...
> 作成 2026-09-15。WRS機上の Claude Code が `wrs_pc_host_instruction.md` に沿って構築し、その完了報告をもとに書いた。
> **この機体で柱Aを進めるときの「今の正しい状態」。** 構築手順そのものは `wrs_pc_host_instruction.md`。
> 2026-09-15 夜: 再エクスポート作業（`wrs_urdf_reexport_instruction.md`）の停止点A の報告で、シェル構成と証明書の変数名が判明したので §3-1・§4 を更新。
...
3. ~~USD の再エクスポート~~ → **完了（2026-09-16 未明）。** URDF 後処理（base +90°、軸反転、limit）→ USD → 配置 → 64 env / 20 iter 完走。機体の約束事は `robot_model_conventions.md`。**commit / push 済み（`6ede75b`、2026-09-16）**

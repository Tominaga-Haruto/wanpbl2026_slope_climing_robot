...
| **2** | 実行1 が **C**、実行2（か3）が **A か B** | ゲインと MIT 経路は効く。**動かないのは HR だけ** | HR の Kp（デプロイ値 10）が HR の静止摩擦に対して低い、が本線。ゲインを勝手に上げず、`actuator_params.md` と学習側の stiffness に戻して設計し直す。任意の 0x13 をやると確度が上がる |

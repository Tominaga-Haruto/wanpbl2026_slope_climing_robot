> **取り下げ済み（2026-09-23）。** この手順は電流中止 1.0 A のままで全身に自重を支えさせるもので、成立し得なかった。全5本が ramp 中に `motion/current abort` で落ちている。根拠 `../reports/2026-09-23_d10-2_result.md`、後継 `d10_3_吊り保持測定_実験手順.md`。本文中の「CSV は先頭軸（0x13）の行だけを持つ」も誤りで、先頭軸は 0x1C である。

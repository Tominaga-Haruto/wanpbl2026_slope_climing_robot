...
**`handoffs/active/2026-09-23_d10-10_吊り紐つき床上立ち.md`**

**D10-9 完了: 足は HAA 約 −8.5°で触れ始め、−10.3 / −11.0°で止まる（R1・R2 で同じ。原点は安定）。当たるのはつま先だけで、本人の判断で許容して進む。** 次は D10-10: 吊り紐を付けたまま床に足を着け、方策なしで立たせて荷重時の保持電流を実測し（R1）、vx 0 の方策を 5 s（R2）。build `D10_10_FLOORSTAND_20260923_1600`（`--floor-limits`: KFE・FFE を 6.0 A、保持 ≤30 s、テスト 143本 OK）。手順 `procedures/d10_10_吊り紐つき床上立ち_実験手順.md`（要承認）。根拠 `reports/2026-09-23_d10-9_result.md`、`reports/2026-09-23_d10-8_result.md`。
...
1. **D10-10（`procedures/d10_10_吊り紐つき床上立ち_実験手順.md`）: 吊り紐つきで床に立たせる（R1 方策なし・荷重時の保持電流、R2 vx 0 の方策 5 s、要承認）。** R1 の実測から床上の中止値の表を作る。歩かせる前に、速度中止 100°/s と slew の扱いを R2 のログから決める。電源の電流を実測する（まだ一度も報告がない）。
...
| 項目 | 読むファイル |
|---|---|
| 次の作業（引き継ぎ書） | `handoffs/active/2026-09-23_d10-10_吊り紐つき床上立ち.md` |
| その実行手順 | `procedures/d10_10_吊り紐つき床上立ち_実験手順.md` |

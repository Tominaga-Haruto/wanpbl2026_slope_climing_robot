...
**`handoffs/active/2026-09-23_d10-9_足の接触角.md`**

**D10-8 完了: 観測の修正で vx 0 の要求は半分以下になった（KFE の SLEW GAP 27〜30°、D10-7 は 64〜89°）。vx 0.2 は D10-7 と同じ（145〜154°）で、吊りではこれ以上分からない。傾きは原因ではない。一方、実機の足は URDF 上の足首間隔 24〜29 cm（HAA −4〜−9°）で当たった。シムは約 18 cm（HAA −14〜−16°）で当てずに歩く。** 床に下ろす前に、D10-9 で実機の足が HAA 何度で当たるかをログで測る（build `D10_9_HAASWEEP_20260923_1500`、`--haa-close-deg`、方策なし、テスト 138本 OK）。次は `procedures/d10_9_HAA閉じ_足の接触角_実験手順.md`（要承認）。根拠 `reports/2026-09-23_d10-8_result.md`。
...
1. **D10-9（`procedures/d10_9_HAA閉じ_足の接触角_実験手順.md`）: 両 HAA を閉じて、実機の足が当たる角度を測る（吊り・方策なし、要承認）。** −14°まで当たらなければ床上の手順書を書く。当たれば WRS 機で足メッシュの距離を計算し、モデル・原点と突き合わせる。電源の電流を実測する（まだ一度も報告がない）。
...
| 項目 | 読むファイル |
|---|---|
| 次の作業（引き継ぎ書） | `handoffs/active/2026-09-23_d10-9_足の接触角.md` |
| その実行手順 | `procedures/d10_9_HAA閉じ_足の接触角_実験手順.md` |

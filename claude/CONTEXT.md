...
**`handoffs/active/2026-09-23_d10-4_全軸slew吊り方策.md`**

**D10-3 完了。** 吊り保持（`--hold-pose`）は 1.0 A のまま完走・保持 ≤0.10 A（ただし脚を手で支えていた）。R3/R4 は ramp を完走し、**方策切替の 1〜2 tick 目で落ちた。原因は送信器の欠陥で、slew が 0x1C だけに掛かり残り9軸が 20 ms で最大 45° 跳んだ。** 送信器を build `D10_4_ALLSLEW_20260923_1200` に直した（全軸 slew 必須・プレアーム・ゲート）。根拠 `reports/2026-09-23_d10-3_result.md`。次は `procedures/d10_4_全軸slew_吊り方策_実験手順.md`（手を離した保持の取り直し → slew つき吊り方策）。
...
1. **D10-4 R0・R1（吊り・手を離して `--hold-pose`・承認不要）→ R2（slew 20 °/s・vx 0）→ R3（slew 30 °/s・vx 0.2）。** 手順 `procedures/d10_4_全軸slew_吊り方策_実験手順.md`。R1b・R2・R3 は要ユーザー承認。**run 中に脚を手で支えない。**
...
4. **床上の電流中止値は D10-4 の実測（手を離した保持・slew つき方策 run の軸別電流）から決める。** 推定で決めない。
...
| 項目 | 読むファイル |
|---|---|
| 次の作業（引き継ぎ書） | `handoffs/active/2026-09-23_d10-4_全軸slew吊り方策.md` |
| その実行手順 | `procedures/d10_4_全軸slew_吊り方策_実験手順.md` |

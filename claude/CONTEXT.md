...
**`handoffs/active/2026-09-23_d10-5_基準姿勢から方策.md`**

**D10-4 完了: 全軸 slew で吊りの方策ループが初めて中止なしで完走（R2・R3）。** ただし脚を閉じた姿勢で `oa` していた（HAA 原点が左約6°・右約12°ずれ）ことと、全軸 0°から始めたせいで方策が分布の外（KFE +178° を要求）だった。**送信器 build `D10_5_STANDPOSE_20260923_1300` で「シムの初期姿勢へ ramp → 保持 → slew つき方策」に変えた。** `oa` は基準姿勢で持ったまま打ち、run の ramp 開始まで持ち続ける。根拠 `reports/2026-09-23_d10-4_result.md`、手順 `procedures/d10_5_基準姿勢から方策_実験手順.md`。
...
1. **D10-5 R0 → R1（初期姿勢へ移して保持のみ、`--stand-only`）→ R2（初期姿勢→方策 vx 0、slew 20）→ R3（vx 0.2、slew 30）。** 手順 `procedures/d10_5_基準姿勢から方策_実験手順.md`。R1〜R3 は要ユーザー承認。R1 の `HOLD POSE RESULT` が床上中止値の根拠。
...
| 項目 | 読むファイル |
|---|---|
| 次の作業（引き継ぎ書） | `handoffs/active/2026-09-23_d10-5_基準姿勢から方策.md` |
| その実行手順 | `procedures/d10_5_基準姿勢から方策_実験手順.md` |

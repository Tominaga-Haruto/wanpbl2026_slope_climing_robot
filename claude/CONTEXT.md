...
**`handoffs/active/2026-09-23_d10-8_観測修正後の吊り方策.md`**

**D10-7 完了: 符号修正後は左右対称に立てた（HFE 1.27 / 1.29 A）。方策の観測の関節角が `joint_pos_rel`（関節角 − 既定角）になっていなかったバグを見つけた（D10-3〜D10-7 のすべての方策 run が該当。golden.npz で確認）。D10-8 で修正し、観測を `_obs.csv` に記録するようにした（build `D10_8_POSREL_20260923_1530`、テスト 132本 OK）。** 次は `procedures/d10_8_観測修正後_吊り方策_実験手順.md`（要承認）。足を閉じるのは、方策がもともと取る姿勢でもある（シムで HAA 約 −15°）。根拠 `reports/2026-09-23_d10-7_result.md`。
...
1. **D10-8（`procedures/d10_8_観測修正後_吊り方策_実験手順.md`）: 観測を直したあとの吊り方策（R1 vx 0 / R2 vx 0.2、要承認）。** `OBSERVATION` の傾き・角速度と、SLEW GAP を D10-7 と比べる。そのあと床上の手順書を書く。電源の電流を実測する。
